...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:actors).group('movies.id').order('COUNT(movies.id) DESC').limit(3)
  # Movie.select(:id, :title, "COUNT(movies.id) as cast_size").joins(:actors).group('movies.id').order('COUNT(movies.id) DESC').limit(3)
